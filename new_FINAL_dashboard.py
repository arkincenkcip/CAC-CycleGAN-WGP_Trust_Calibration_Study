...
def load_svm_classifier(svm_subdir: str = "baseline"):
    svm_path = PROJECT_ROOT / "resources" / "svm_models" / svm_subdir / "svm_fault_classifier.pkl"
...
def load_svm_scaler(svm_subdir: str = "baseline"):
    scaler_path = (
        PROJECT_ROOT / "resources" / "svm_models" / svm_subdir / "svm_fault_classifier_scaler.pkl"
    )
...
STUDY_POOLS_PATH = PROJECT_ROOT / "study_pools.json"
...
# One row per event (trial_start / prediction_shown / thumbnail_select /
#                    trust_decision / confidence_rating / trial_end)
EVENT_LOG_COLUMNS = [
    # ── identity ──────────────────────────────────────────────────────────────
    "participant_id", "participant_name", "pool_id", "scenario",
    # ── trial coordinates ─────────────────────────────────────────────────────
    "trial_id",           # "B{scenario_block}_T{position:02d}"
    "block_number",       # scenario number: S1→1, S2→2, S3→3
    "trial_position",     # 1-indexed position within scenario
    ...
    # ── event ─────────────────────────────────────────────────────────────────
    "event_type",         # trial_start | prediction_shown | thumbnail_select |
                          #               trust_decision | confidence_rating | trial_end
    "event_value",        # decision string / confidence int / None
    "timestamp",          # datetime of this event (Excel datetime cell)
    # ── class / NRMSE (thumbnail_select + prediction_shown) ───────────────────
    "class_id", "nrmse_rank", "nrmse_score",
    # ── decision outcome (trial_end) ──────────────────────────────────────────
    "decision", "confidence_rating", "appropriate_decision",
    # ── timing ────────────────────────────────────────────────────────────────
    "elapsed_seconds",       # wall-clock seconds from trial_start to trial_end
    "trial_start_ts",        # datetime: trial loaded (Excel datetime cell)
    "predict_ts",            # datetime: after SVM prediction
    "generate_ts",           # datetime: after CycleGAN generation (S3 only)
    "time_to_decision_s",    # seconds from trial_start to trust_decision
    "time_to_generate_s",    # seconds from trial_start to CycleGAN done (S3)
    # ── top-3 NRMSE hypotheses (trial_end) ────────────────────────────────────
    "top1_label", "top1_score", "top2_label", "top2_score",
    "top3_label", "top3_score",
    # ── all NRMSE scores (S3 only) ────────────────────────────────────────────
    ...
    "s3_nrmse_variance",  # spread of NRMSE scores across 9 classes
    # ── S2 reference NRMSE ────────────────────────────────────────────────────
    "s2_ref_nrmse",       # NRMSE score of the single real reference shown in S2
    # ── experimental: real-reference comparison ───────────────────────────────
    "compare_with_real",  # NRMSE of a real training sample of the TRUE class
    # ── misc ──────────────────────────────────────────────────────────────────
    "svm_variant",
]
...
def _log_study_event(event_type: str, **kwargs) -> None:
    """Buffer one event row in session state — flushed to disk on Submit & Continue."""
    ss  = st.session_state
    ...
    top3 = ss.get("study_trial_top3") or []
    def _top(i, field):
        return top3[i].get(field) if i < len(top3) else None
    ...
    row = {
        ...
        "trial_start_ts":   datetime.fromtimestamp(ss["study_trial_start_ts_ms"] / 1000)
                            if ss.get("study_trial_start_ts_ms") else None,
        "predict_ts":       datetime.fromtimestamp(ss["study_predict_ts_ms"] / 1000)
                            if ss.get("study_predict_ts_ms") else None,
        "generate_ts":      datetime.fromtimestamp(ss["study_generate_ts_ms"] / 1000)
                            if ss.get("study_generate_ts_ms") else None,
        "time_to_decision_s": round(
            (ss["study_predict_ts_ms"] - ss["study_trial_start_ts_ms"]) / 1000, 3)
            if ss.get("study_predict_ts_ms") and ss.get("study_trial_start_ts_ms") else None,
        "time_to_generate_s": round(
            (ss["study_generate_ts_ms"] - ss["study_trial_start_ts_ms"]) / 1000, 3)
            if ss.get("study_generate_ts_ms") and ss.get("study_trial_start_ts_ms") else None,
        "top1_label": _top(0, "label"), "top1_score": _top(0, "score"),
        "top2_label": _top(1, "label"), "top2_score": _top(1, "score"),
        "top3_label": _top(2, "label"), "top3_score": _top(2, "score"),
        "svm_variant": ss.get("svm_subdir", "baseline"),
    }

    # ── S3: per-class NRMSE scores + variance ────────────────────────────────
    s3_scores = ss.get("study_trial_nrmse_scores") or []
    s3_labels = ss.get("study_trial_s3_labels") or []
    nrmse_map = {}
    if len(s3_scores) == 9 and len(s3_labels) == 9:
        for label, score in zip(s3_labels, s3_scores):
            nrmse_map[f"s3_nrmse_class{int(label)}"] = float(score)
    for c in range(9):
        row[f"s3_nrmse_class{c}"] = nrmse_map.get(f"s3_nrmse_class{c}")
    row["s3_nrmse_variance"] = float(np.var(s3_scores)) if len(s3_scores) > 1 else None
    ...
    if scenario == "S2":
        ...
        if s2_refs is not None and sig is not None and len(s2_refs) > 0:
            s2_ref_nrmse = float(sim_nrmse_as_displayed(sig, s2_refs[0]))
    row["s2_ref_nrmse"] = s2_ref_nrmse

    # ── [_COMPARE_WITH_REAL] ─────────────────────────────────────────────────
    row["compare_with_real"] = ss.get("study_trial_real_true_nrmse") if _COMPARE_WITH_REAL else None
...
# Fixed pool assignment per scenario (all participants use same mapping)
SCENARIO_POOL = {"S1": "C", "S2": "B", "S3": "A"}
...
def _scenario_trial_count(scenario: str, pool_data: Optional[dict]) -> int:
    if pool_data is None:
        return 0
    pool = pool_data.get(SCENARIO_POOL[scenario], {})
...
def _init_study_state() -> None:
    ...
    ss.setdefault("study_trial_real_true_nrmse", None)   # _COMPARE_WITH_REAL
    ...
    ss.setdefault("shuffle_config",             1)
...
def _start_scenario(scenario: str) -> None:
    import random
    ss      = st.session_state
    cfg     = ss.get("shuffle_config", 1)
    rng     = random.Random(cfg * 1000 + _SCENARIO_SEED_OFFSET[scenario])
    pool_id = SCENARIO_POOL[scenario]
...
def _start_practice() -> None:
    import random
    ss  = st.session_state
    rng = random.Random(ss.get("shuffle_config", 1) * 1000)
...
def _study_load_current_trial(tester, svm_model) -> None:
    ...
    # ── log trial_start (now that all metadata is in session state) ───────────
    _log_study_event("trial_start")

    # ── log prediction_shown ──────────────────────────────────────────────────
    nrmse_rank_pred  = trial.get("nrmse_rank_predicted")
    nrmse_score_pred = trial.get("nrmse_score_predicted")
    _log_study_event(
        "prediction_shown",
        class_id=pred,
        nrmse_rank=nrmse_rank_pred,
        nrmse_score=nrmse_score_pred,
    )
...
def _study_on_trust_decision(decision: str) -> None:
    ...
    ss = st.session_state
    _log_study_event("trust_decision", event_value=decision, decision=decision)
...
def _study_on_confidence(confidence: int) -> None:
    ...
    _log_study_event("confidence_rating", event_value=confidence,
    ...
    ss.study_trial_real_true_nrmse = None  # _COMPARE_WITH_REAL
...
def _render_participant_entry() -> None:
    ...
    with col:
        ...
        cfg = st.selectbox(
            ...
            index=st.session_state.get("shuffle_config", 1) - 1,
            format_func=lambda x: f"Configuration {x}",
            help="Each number produces a fixed, reproducible trial order. "
                 "If the session is interrupted, re-enter the same configuration to resume.",
        )
        ...
        for i, sc in enumerate(["S1", "S2", "S3"]):
            ...
            with cols[i + 1]:
                st.markdown(
                    ...
                    f"<div class='sc-row'>Pool &nbsp;<b>{info['pool']}</b></div>"
