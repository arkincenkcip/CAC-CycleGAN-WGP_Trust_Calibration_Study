...
OUT_DIR = PROJECT_ROOT / "resources" / "models"
OUT_DIR.mkdir(parents=True, exist_ok=True)
out_path_AB = OUT_DIR / 'generator_AB'
out_path_BA = OUT_DIR / 'generator_BA'
...
class Generator(nn.Module):
    ...
    def _conv_block(self, in_channels, out_channels, kernel_size=5):
        """Encoder block: Conv2d -> LeakyReLU -> InstanceNorm (no running stats)"""
    ...
    def _deconv_block(self, in_channels, out_channels, kernel_size=5, dropout_rate=0):
        """Decoder block: Upsample -> Conv2d -> ReLU -> InstanceNorm"""
        layers = [
        ...
        layers.append(nn.InstanceNorm2d(out_channels, affine=True, track_running_stats=False))
...
class CycleGAN:
    ...
    def train(self, epochs, data_path, save_dir=OUT_DIR / 'epoch_models', best_model_name = 'best_cyclegan_model.pth', add_quantity=995,
              save_interval=10, svm_function=None):
        """
        ...
        high_accuracy_models = []  # to record all >90% models
        ...
        domain_A_train_X = domain_A_train_X
        domain_A_train_Y = domain_A_train_Y
        
        num_batches = int(domain_A_train_X.shape[0] / self.batch_size)
        
        print(f"Training data loaded:")
        print(f"  Domain A (for training): {domain_A_train_X.shape}")
        ...
        best_accuracy = 0.0
        accuracy_list = []
        high_accuracy_models = []  # store all models with accuracy >= 90%
        ...
        for epoch in range(epochs):
            ...
            for batch_i in range(num_batches):
                ...
                batch_samples_A = torch.FloatTensor(
                    domain_A_train_X[start_i:end_i]
                ).to(self.device)
                batch_labels_A = torch.LongTensor(
                    domain_A_train_Y[start_i:end_i].reshape(-1, 1)
                ).to(self.device)
            ...
            # --- 1️⃣ TRAIN-MODE GENERATOR EVALUATION (original behavior) ---
            self.g_AB.train()
            current_accuracy_train = samlpe_generation_feed_svm(
                ...
                domain_A_train_x=domain_A_train_X,
                domain_A_train_y=domain_A_train_Y,
                domain_B_train_x=domain_B_train_X,
                domain_B_train_y=domain_B_train_Y,
                c=0.2,
                g=0.001
            )

            # --- 2️⃣ EVAL-MODE GENERATOR EVALUATION (for comparison) ---
            self.g_AB.eval()
            with torch.no_grad():
                current_accuracy_eval = samlpe_generation_feed_svm(
                    add_quantity=add_quantity,
                    test_x=test_X,
                    test_y=test_Y,
                    generator=self.g_AB,
            ...
            # --- Logging results ---
            print(f"\n[Epoch {epoch+1}/{epochs}] Accuracy (train-mode): {current_accuracy_train:.4f}")
            print(f"[Epoch {epoch+1}/{epochs}] Accuracy (eval-mode):  {current_accuracy_eval:.4f}")

            # Track the 'train-mode' accuracy as your main metric
            current_accuracy = current_accuracy_train



            accuracy_list.append({
                'epoch': epoch + 1,
                'train_mode_acc': current_accuracy_train,
                'eval_mode_acc': current_accuracy_eval
            })

            # --------------------------------------------------------
            # Periodic checkpoint: save every N epochs (save_interval)
            # --------------------------------------------------------
            # if (epoch + 1) % save_interval == 0:
            #     periodic_path = os.path.join(save_dir, f"cyclegan_epoch_{epoch+1}.pth")
            #     # Save a full checkpoint so it is compatible with the tester script
            #     torch.save({
            #         'epoch': epoch + 1,
            ...
            # ========================================================
            # Save all models with accuracy >= 90%
            # ========================================================
            if current_accuracy_eval  >= 0.85:
                high_acc_dir = os.path.join(save_dir, "high_accuracy_models")
                os.makedirs(high_acc_dir, exist_ok=True)

                high_acc_path = os.path.join(
                    high_acc_dir,
                    f"model_acc_{current_accuracy_eval*100:.2f}_epoch_{epoch+1}.pth"
                )

                checkpoint = {
                    'epoch': epoch + 1,
                    'accuracy': current_accuracy_eval,
                    'g_AB_state_dict': self.g_AB.state_dict(),
                    'g_BA_state_dict': self.g_BA.state_dict(),
                    'd_1_state_dict': self.d_1.state_dict(),
                    'd_2_state_dict': self.d_2.state_dict(),
                ...
                torch.save(checkpoint, high_acc_path)
                high_accuracy_models.append({
                    'epoch': epoch + 1,
                    'accuracy': current_accuracy,
                    'path': high_acc_path
                })
                print(f"🔥 Saved high-accuracy model ({current_accuracy_eval*100:.2f}%) to {high_acc_path}")

            # ========================================================
            # Standard best-model saving logic
            # ========================================================
            if current_accuracy_eval > best_accuracy:
                best_accuracy = current_accuracy_eval
            ...
            print("\nRecent accuracy history (last 10 epochs):")
            for entry in accuracy_list[-10:]:
                print(f"  Epoch {entry['epoch']:4d} | "
                    f"Train: {entry['train_mode_acc']:.4f} | "
                    f"Eval: {entry['eval_mode_acc']:.4f}")
            print("="*80 + "\n")
        
        # ========================================================
        # Save summary list of all high-accuracy models to file
        # ========================================================
        summary_path = os.path.join(save_dir, "high_accuracy_models_eval_based.txt")

        with open(summary_path, "w", encoding="utf-8") as f:
            f.write("List of all models with accuracy ≥ 90%\n")
            f.write("="*70 + "\n")
            if len(high_accuracy_models) == 0:
                f.write("No models reached ≥ 90% accuracy.\n")
            else:
                for m in high_accuracy_models:
                    f.write(f"Epoch: {m['epoch']:4d} | Accuracy: {m['accuracy']*100:.2f}% | Path: {m['path']}\n")

        print("\n" + "="*80)
        print("📊 Summary of all high-accuracy models (≥90%):")
        if len(high_accuracy_models) == 0:
            print("  No models reached ≥90% accuracy.")
        else:
            for m in high_accuracy_models:
                print(f"  Epoch {m['epoch']:4d} | Acc: {m['accuracy']*100:.2f}% | Path: {m['path']}")
        print(f"\n📝 Full list saved to: {summary_path}")
        print("="*80)
...
if __name__ == '__main__':
    ...
    if os.path.exists(data_path):
        gan.train(
            epochs=1000,
            data_path=data_path,
            save_dir=OUT_DIR / 'epoch_models',
            save_interval=10
        )
        
        # Save final model
        gan.save_model(OUT_DIR/'cyclegan_final.pth')
    else:
